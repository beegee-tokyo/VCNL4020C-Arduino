...
/**
 * @brief Product ID Revision Register
 * |_                     |_ 
 * |------                |-------
...
/**
 * @brief Rate of Biosensor Measurement Register
 * Bit 2 -- Bit 0
 * Rate of biosensor measurement
 */
#define BIO_SENS_RATE 0x82

#define BIO_SENS_RATE_1_95 0b00000000 ///< Bio sensor data rate 1.95 measurements/s (DEFAULT)
#define BIO_SENS_RATE_3_9 0b00000001  ///< Bio sensor data rate 3.90625 measurements/s
#define BIO_SENS_RATE_7_8 0b00000010  ///< Bio sensor data rate 7.8125 measurements/s
#define BIO_SENS_RATE_16_3 0b00000011 ///< Bio sensor data rate 16.625 measurements/s
#define BIO_SENS_RATE_31_3 0b00000100 ///< Bio sensor data rate 31.25 measurements/s
#define BIO_SENS_RATE_62_5 0b00000101 ///< Bio sensor data rate 62.5 measurements/s
#define BIO_SENS_RATE_125 0b00000110  ///< Bio sensor data rate 125 measurements/s
#define BIO_SENS_RATE_250 0b00000111  ///< Bio sensor data rate 250 measurements/s

/**
 * @brief LED Current Setting fro Biosensor Mode
...
/**
 * @brief Ambient Light Parameter Register
 * |_                        |_ 
 * |------                   |-------
 * |Bit 7                    | Bit 6 -- Bit 4
...
/**
 * @brief Ambient light result register
 * High byte
 */
...
/**
 * @brief Bio sensor result register
 * High byte
 */
 ...
 * |__________________|__________________|______________|_____________
 * |------------------|------------------|--------------|------
 * | Bit 7 -- Bit 5	  | Bit 4            | _            | _
 * | Int count exceed | unused           | _            | _
 * | Bit 3            | Bit 2            | Bit 1        | Bit 0
...
/** 
 * @brief Biosensor Modulator Timing Adjustment
 *
 * - **!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!**
...
public:
	 ...
	 * @return result of request
	 */
	bool getBioDataRate(uint8_t *dataRate);
	/**
	 * Set LED current 
	 * @param ledCurrent
...
private:
	TwoWire *_i2c; ///< Pointer to I2C class

	int _addr = VCNL4020C_ADDR; ///< Sensor I2C address

	uint16_t _lowThresh = 0;  ///< User defined lower treshold
	uint16_t _highThresh = 0; ///< User defined upper treshold

	uint8_t regValue = 0; ///< Temporary register value

	bool _intMeasurementBio = false; ///< Flag if BIO interrupts are enabled
	bool _intMeasurementALS = false; ///< Flag if ALS interrupts are enabled
	bool _intThreshold = false;		 ///< Flag if Treshold interrupts are enabled
	 ...
	 */
	void (*_sensorInt)() = NULL; ///< Pointer to callback function

	int _intPin = -1; ///< GPIO connected to interrupt of VCNL4020
};
#endif
